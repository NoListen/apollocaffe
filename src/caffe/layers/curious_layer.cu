...
void CuriousLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  LOG(INFO) << "gpu_data get!";
  for (int i = 0; i < bottom.size(); ++i) {
    const Dtype* bottom_data = bottom[i]->gpu_data();
    //memset(top[i]->mutable_cpu_data(),0, top[i]->count() * sizeof(Dtype));

    Dtype* top_data = top[i]->mutable_gpu_data();
    cudaMemset(top_data, 0, top[i]->count() * sizeof(Dtype));
    //memset(top_data,0, top[i]->count() * sizeof(Dtype));
    LOG(INFO)<<"top bottom get";
    for (int n = 0; n < this->num_; ++n) {
      this->forward_gpu_gemm(bottom_data + bottom[i]->offset(n), quantized_book, quantized_indicatior,
