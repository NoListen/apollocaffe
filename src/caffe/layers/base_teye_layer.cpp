...
void BaseTeyeLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  CHECK_EQ(4, bottom[0]->num_axes()) << "Input must have 4 axes, "
      << "corresponding to (num, channels, height, width)";
  // Configure the kernel size, padding, stride, and inputs.
  TeyeParameter conv_param = this->layer_param_.teye_param();
  ...
  kmap_length_ = conv_param.kl();
  // ignore the group at first!
  // if group would be introduced, I'll use several param
  ...
  bias_term_ = this->layer_param_.teye_param().bias_term();
  if (this->blobs_.size() > 0) {
    ...
    vector<int> shrink_weight_shape_;
...
template <typename Dtype>
void BaseTeyeLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  CHECK_EQ(4, bottom[0]->num_axes()) << "Input must have 4 axes, "
      << "corresponding to (num, channels, height, width)";
  num_ = bottom[0]->num();
  ...
  if (reverse_dimensions()) {
    col_buffer_.Reshape(1, kmap_length_, height_, width_);
  } else {
    col_buffer_.Reshape(1, kmap_length_, height_out_, width_out_);
  }
...
template <typename Dtype>
void BaseTeyeLayer<Dtype>::forward_cpu_gemm(const Dtype* input,
    const Dtype* weights, Dtype* output, const Dtype *kernel_map, bool skip_im2col) {
  const Dtype* col_buff = input;
  if (!is_1x1_) {
    if (!skip_im2col) {
      teye_im2col_cpu(input, col_buffer_.mutable_cpu_data(), kernel_map);
    }
...
template <typename Dtype>
void BaseTeyeLayer<Dtype>::forward_cpu_bias(Dtype* output,
    const Dtype* bias) {
...
template <typename Dtype>
void BaseTeyeLayer<Dtype>::backward_cpu_gemm(const Dtype* output,
  ...
  for (int g = 0; g < group_; ++g) {
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, kmap_length_ / group_,
        conv_out_spatial_dim_, conv_out_channels_ / group_,
        (Dtype)1., weights + weight_offset_ * g, output + output_offset_ * g,
        (Dtype)0., col_buff + col_offset_ * g);
  }
  if (!is_1x1_) {
    teye_col2im_cpu(col_buff, input, kernel_map);
  }
}

template <typename Dtype>
void BaseTeyeLayer<Dtype>::weight_cpu_gemm(const Dtype* input,
...
template <typename Dtype>
void BaseTeyeLayer<Dtype>::backward_cpu_bias(Dtype* bias,
    const Dtype* input) {
